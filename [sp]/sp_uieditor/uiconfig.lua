...
function UIDescriptor:load( xml )
    ...
    local id = tonumber( xmlNodeGetAttribute( xml, "id" ) )
    if id and id >= 0 then
        self.id = id
    end
...
function UIDescriptor:save( xml )
    ...
    if type( self.id ) == "number" then
        xmlNodeSetAttribute( xml, "id", self.id )
    end
...
function UIDescriptor:insertChild( frame )
    ...
    -- Оповещаем холст
    self.canvas:onFrameCreated( frame )
...
function UIDescriptor:onCursorMove( ax, ay )
    local locked = self.canvas.focusLock
    if not locked then
        self.selected = false
    end

    local force = locked and self.selected
...
function UIList:load( xml )
    local scrollbarID = tonumber( xmlNodeGetAttribute( xml, "scrollbar" ) )
    if scrollbarID then
        self:setScrollbarFrameID( scrollbarID )
    end
...
function UIList:save( xml )
    if type( self.scrollbarID ) == "number" then
        xmlNodeSetAttribute( xml, "scrollbar", self.scrollbarID )
    end
...
function UIList:forward()
    ...
    local y = self.ty + self.scrollValue
    local height = 0
    ...
    local scrollbarFrame = self.canvas:getFrameByID( self.scrollbarID )
    if scrollbarFrame then
        local exceedHeight = math.max( self.contentHeight - self.th, 0 )
        local scale = math.pow( self.th / ( exceedHeight + self.th ), 2 )

        scrollbarFrame:setScrollScale( scale )
    end
end
...
function UIList:setScrollValue( scrollValue )
    local exceedHeight = math.max( self.contentHeight - self.th, 0 )
    scrollValue = math.clamp( -exceedHeight, 0, scrollValue )

    if scrollValue ~= self.scrollValue then
        ...
        local scrollbarFrame = self.canvas:getFrameByID( self.scrollbarID )
        if scrollbarFrame then
            local pos = exceedHeight > 0 and scrollValue / -exceedHeight or 0 -- Избегаем NaN
            
            scrollbarFrame:setScrollPosition( pos )
        end
    end
end

function UIList:setScrollbarFrameID( id )
    self.scrollbarID = tonumber( id )
end

function UIList:setGapHeight( gap )
    self.gapHeight = tonumber( gap ) or 0

    return self
end

-- Вызов от UIScrollBar
function UIList:onScrollEvent( pos )
    local exceedHeight = math.max( self.contentHeight - self.th, 0 )

    self.scrollValue = exceedHeight > 0 and -exceedHeight * math.clamp( 0, 1, pos ) or 0
...
function UIList:onKey( key, pressed )      
    UIDescriptor.onKey( self, key, pressed )

    if key == "mouse_wheel_down" then
        self:setScrollValue( self.scrollValue - 20 )
...
function UIRenderTarget:update()
    ...
    --[[
        Пересоздаем только при необходимости
    ]]
    local recreate = not self._rt or ( self.tw ~= self._lastTw or self.th ~= self._lastTh )
    if recreate then
        if self._rt then
            destroyElement( self._rt )
        end
        ...
        self._lastTw = self.tw
        self._lastTh = self.th
...
function UIEditField:draw()
    ...
    local text = tostring( self.clippedText )
    if self.masked then
        text = _textMask( text )
    end
...
function UIScrollBar:create( name, x, y, width, height )
    ...
    frame.stateIdx = UIBTN_DEFAULT
    frame.scrollPos = 0
    frame.scrollScale = SCROLL_MIN_SCALE
    frame.dependentId = nil

    return setmetatable( frame, UIScrollBarMT )
end

function UIScrollBar:load( xml )
    local textureName = xmlNodeGetAttribute( xml, "texture" )
    if textureName then
        self:setTexture( textureName )
    end

    local dependentId = tonumber( xmlNodeGetAttribute( xml, "dependent" ) )
    if dependentId then
        self:setDependentFrameId( dependentId )
    end
...
function UIScrollBar:save( xml )
    if self.tex then
        xmlNodeSetAttribute( xml, "texture", tostring( self.texName ) )
    end

    if type( self.dependentId ) == "number" then
        xmlNodeSetAttribute( xml, "dependent", self.dependentId )
    end
...
function UIScrollBar:draw()
    ...
    local section = self.sections[ self.stateIdx ] or self.sections[ UIBTN_DEFAULT ]
    if self.tex and section then
        local height = self.th * self.scrollScale
        local trackHeight = self.th - height
        local pos = trackHeight > 0 and trackHeight * self.scrollPos or 0

        section:draw( self.tex, self.tx, self.ty + pos, self.tw, height, 0, 0, 0, color )
    end
end

function UIScrollBar:onVisibleChange( visible )
    self.stateIdx = UIBTN_DEFAULT
end

function UIScrollBar:onCursorEnter( ax, ay )
    self.stateIdx = UIBTN_SELECTED
end

function UIScrollBar:onCursorExit( ax, ay )
    self.stateIdx = UIBTN_DEFAULT
end
...
function UIScrollBar:onCursorClick( btn, state, ax, ay )
    ...
    do
        local height = self.th * self.scrollScale
        local trackHeight = self.th - height
        local pos = trackHeight * self.scrollPos
        local sx = self.tx
        local sy = self.ty + pos

        self.clicked = self.selected and state == "down"
        self.clickedBiasX = sx - ax
        self.clickedBiasY = sy - ay
    end
end
...
function UIScrollBar:onCursorMove( ax, ay )
    ...
    if self.selected and self.clicked then
        local height = self.th * self.scrollScale
        local trackHeight = self.th - height

        local sx = math.clamp( 0, trackHeight, ( ay - self.ty ) + self.clickedBiasY )
        local pos = sx / trackHeight

        self:setScrollPosition( pos )
        
        local dependentFrame = self.canvas:getFrameByID( self.dependentId )
        if dependentFrame then
            dependentFrame:onScrollEvent( pos )
        end
    end
end

function UIScrollBar:setTexture( textureName )
    local tex = exports.sp_assets:xrLoadAsset( textureName )
    if tex then
        self.tex = tex
        self.texName = textureName -- Для сохранения
...
function UIScrollBar:setTextureSection( stateIdx, sectionName )
    local section = xrTextureSections[ sectionName ]
    if section then
        self.sections[ stateIdx ] = section
    else
...
function UIScrollBar:setScrollPosition( pos )
    self.scrollPos = math.clamp( 0, 1, pos )
end

function UIScrollBar:setScrollScale( scale )
    self.scrollScale = math.clamp( SCROLL_MIN_SCALE, 1, scale )
end

function UIScrollBar:setDependentFrameId( id )
    self.dependentId = tonumber( id )
end
...
function UICanvas:getFrameByID( id )
    if type( id ) == "number" then
        return self.frameIDs[ id ]
    end
end

function UICanvas:onFrameCreated( frame )
    if frame.id and frame.id > 0 then
        if self.frameIDs[ frame.id ] then
            outputDebugString( "ID " .. frame.id .. " уже присутствует на полотне!", 2  )
            return
        end

        self.frameIDs[ frame.id ] = frame
    else
        local nextID = self.lastFrameID + 1

        --[[
            При достижении предела числового значения
            пытаемся рекупирировать ID начиная с 1
        ]]
        if nextID >= INT32_MAX then
            nextID = 1
        end

        while self.frameIDs[ nextID ] ~= nil do
            nextID = nextID + 1
        end

        frame.id = nextID
        self.lastFrameID = nextID
        self.frameIDs[ nextID ] = frame
    end    
end
...
function UICanvas:load( xml )
    ...
    -- Сбрасываем коллекцию ID
    self.frameIDs = setmetatable( {}, { __mode = "kv" } )
    self.lastFrameID = 0
...
function xrCreateUICanvas( screenX, screenY, screenWidth, screenHeight )  
    local canvas = {
        ...
        inputFrame = nil,
        backspacing = false,
        frameIDs = setmetatable( {}, { __mode = "kv" } ),
        lastFrameID = 0
    }
    setmetatable( canvas, UICanvasMT )

    local frame = UIDescriptor:create( "Canvas", screenX, screenY, screenWidth, screenHeight )
    canvas:onFrameCreated( frame )
    frame.canvas = canvas
