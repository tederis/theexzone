...
float4 PixelShaderFunction(PSInput PS) : COLOR0
{
	...
	{	
		...
		float Attenuation = saturate(1.0f - length(Light) / 6.0f);
		
		l_base += Attenuation*Attenuation * PointLightColor[i] * nightFactor; 
	}
	...
	for(int i = 0; i < spotLightsNum; i++)
	{
		...
		l_base += cone * Attenuation*Attenuation * SpotLightColor * nightFactor;
	}

    // lighting
	float3	l_hemi 	= PS.c0;			// hemi
	float3 	l_sun 	= PS.c1 * 0.34f;			// sun color
	float3	light	= l_base + L_ambient + l_sun + l_hemi;

    float3	final 	= (light*t_base.rgb);
