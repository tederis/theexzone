...
addEventHandler( "onClientKey", root,
	function( key )
		if key == "mouse_wheel_up" then			
			options.slowMaxSpeed = math.min( options.slowMaxSpeed + 0.01, 5 )
		elseif key == "mouse_wheel_down" then
			options.slowMaxSpeed = math.max( options.slowMaxSpeed - 0.01, 0.001 )
		end
	end
)
...
addCommandHandler( "xfreecam",
	function()
		if isFreecamEnabled() then
			setFreecamDisabled()
			setCameraTarget( localPlayer )
			for _, controlName in ipairs( controlTable ) do
				if prevControls[ controlName ] then
					toggleControl( controlName, prevControls[ controlName ] ) 
				end
			end
			...
			setFreecamEnabled()
			for _, controlName in ipairs( controlTable ) do
				prevControls[ controlName ] = isControlEnabled( controlName )
