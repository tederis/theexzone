...
local function onPedDamage( attacker, weapon, bodypart, loss )
    ...
    if agent then
        agent:onDamage( attacker, weapon, bodypart, loss )
    end    
end
...
local function onRemoteAgentAction( actionHash, ... )
    local agent = xrPedAgents[ source ]
    if agent then
...
addEventHandler( "onClientCoreStarted", root,
    function()
        ...
        addEvent( EClientEvents.onClientRemoteAgentAction, true )
        addEventHandler( EClientEvents.onClientRemoteAgentAction, resourceRoot, onRemoteAgentAction )
